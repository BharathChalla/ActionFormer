...
def csv_to_json(split_type):
    ...
    video_splits_dir = os.path.join(dataset_dir, "video_splits", split_type, "normal")

    step_idx_description_path = os.path.join(annotations_dir, "step_idx_description.csv")
    step_annotations_path = os.path.join(annotations_dir, "all_step_annotations.csv")
    ...
    step_idx_description = pd.read_csv(step_idx_description_path, header=None, names=["step_id", "step_description"])
    data_subsets = {
        "train": "Training",
        "val": "Validation",
        "test": "Test",
    }
    steps_count = max(step_idx_description["step_id"].values) + 1
    error_dataset_version = f"PTGDataset-{fps}fps-{steps_count}Steps"
        ...
        for recipe_id in os.listdir(data_split_dir):
            ...
            for video_name in os.listdir(recipe_dir):
                ...
                for i in range(len(annotations)):
                    ...
                    has_error = annotations.iloc[i]["has_errors"]
